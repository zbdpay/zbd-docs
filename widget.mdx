...
The ZBD Widget is an embeddable iframe that supports the cashout experience for your users — OTP verification, identity verification, bank account linking, and bank payouts. Your backend creates and funds widget users, creates sessions, and records acceptance of required disclosures.
...
Your application must present and record required widget disclosures outside
the iframe. Requirements depend on your agreement with ZBD and the services
enabled for your project. ACH cashout requires acceptance of the current
Electronic Funds Transfer disclosure (type `6`).

ZBD presents and records acceptance of its Terms of Service and Privacy Policy
separately. Do not submit those disclosure types through the widget endpoints.
See [Disclosure Agreements](/widget/disclosures) for the complete flow.

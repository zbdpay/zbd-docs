...
Before integrating, create a [ZBD Developer Dashboard account](/get-started/create-account), [create a project](/get-started/create-project), and copy your project API key from the project's [API section](/get-started/api-keys). You also need an HTTPS webhook endpoint ready for widget server events.

<Warning>The ZBD Widget requires a webhook URL. ZBD sends signed cashout status updates to this endpoint, and Create Session requires `webhook_url`.</Warning>

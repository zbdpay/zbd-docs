...
Use the sandbox API host for sandbox-only routes:

```bash
SANDBOX_API_BASE_URL=https://sandbox.zbdpay.com
```

For environment-specific testing, use the matching sandbox host, such as `https://sandbox-x1.zbdpay.com`.
...
```bash
curl -X POST $SANDBOX_API_BASE_URL/api/v1/cashout/sandbox/users \
...
```bash
curl -X POST $SANDBOX_API_BASE_URL/api/v1/widget/users/fund \
  -H "apikey: YOUR_SANDBOX_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST $SANDBOX_API_BASE_URL/api/v1/widget/users/session \
  -H "apikey: YOUR_SANDBOX_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "test@yourcompany.com",
...
Use these sandbox-only controls with `SANDBOX_API_BASE_URL` and `apikey: YOUR_SANDBOX_API_KEY` to put a test user or monetary account into a known state before opening the widget.

For user rewards, use `POST /api/v1/widget/users/fund`. The monetary-account helpers below are for state setup when you already have the monetary account ID.
...
In sandbox, the `cashout.completed` webhook can fire shortly after `cashout.initiated` since there's no real ACH settlement delay. See [Webhook Events](/widget/webhooks) for the full event reference.

## Sandbox Limitations

- **No real ACH transfers** — cashouts are simulated and can settle faster than production
- **No real KYC** — use sandbox controls to set KYC states

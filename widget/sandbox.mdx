...
| **Disclosures** | Same status and acceptance model; bypass settings may skip the type `6` ACH check | Electronic Funds Transfer type `6` blocks ACH until accepted |
...
<Note>
  Electronic Funds Transfer disclosure type `6` can be tested in sandbox with the same flow as production. If you need to verify that ACH cashout is blocked when type `6` is missing, make sure your sandbox setup is not using a bypass mode that skips the disclosure gate. Types `1` through `5` do not affect widget eligibility.
</Note>

...
Session creation is not blocked by disclosure types `1` through `5`, and those
types can be ignored for widget eligibility. The session can report outstanding
disclosures as a read-only status, but the widget does not render documents or
collect acceptance.

Before an ACH cashout can succeed, your application must present and record the
latest Electronic Funds Transfer disclosure (type `6`). This requirement is
enforced when the ACH cashout is submitted, not when the session is created. See
[Disclosure Agreements](/widget/disclosures).

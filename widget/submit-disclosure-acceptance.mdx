---
title: "Submit Disclosure Acceptance"
description: "Record disclosure acceptance for a widget user."
api: "POST https://api.zbdpay.com/api/v1/widget/users/{userId}/disclosures"
---

## Description

Records the user's acceptance of the latest Electronic Funds Transfer
disclosure (EFT, type `6`). Use this from your server after your application has
presented the EFT document and collected the user's explicit acceptance. Do not
call this endpoint from a browser, mobile client, game client, or WebView.

## Configuration

### Header Parameters

<ParamField required header="apikey" type="string">
  Your ZBD project API key.
</ParamField>

<ParamField initialValue="application/json" header="Content-Type" type="string">
  Content Type
</ParamField>

### Path Parameters

<ParamField required path="userId" type="string">
  The ZBD user ID returned when the widget user is created or resolved.
</ParamField>

### Body Parameters

<ParamField required body="acceptedDisclosureTypeIds" type="array">
  Widget disclosure type IDs accepted by the user. Submit `[6]` to record the latest EFT version.
</ParamField>

<RequestExample>
```bash cURL
curl -X POST https://api.zbdpay.com/api/v1/widget/users/{userId}/disclosures \
  -H "apikey: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "acceptedDisclosureTypeIds": [6]
  }'
```
</RequestExample>

<ResponseExample>
```json 201
{
  "success": true,
  "message": "Publisher user disclosure agreements recorded successfully.",
  "data": {
    "acceptedDisclosureTypeIds": [6]
  },
  "error": null
}
```
</ResponseExample>

## Widget Disclosure Type ID

| Disclosure type | Type ID |
|---|---:|
| Electronic Funds Transfer | `6` |

ACH cashout requires acceptance of the current EFT version.

## Backward compatibility

The endpoint processes only type `6`. Unsupported type IDs are ignored, and
acceptance is not recorded for those types. A request containing `[2, 3, 6]`
records EFT only and returns `[6]`. A request containing only unsupported IDs
returns `201` with an empty `acceptedDisclosureTypeIds` array.

New and updated integrations should submit only `[6]`.

## Existing Users

If a user already exists and needs to accept a current disclosure, call this endpoint. Do not rely on an idempotent user-create call to update disclosure acceptance for an existing user.

## Errors

| Status | Reason |
|---|---|
| `400` | `acceptedDisclosureTypeIds` is empty or missing |
| `401` | Missing or invalid API key |
| `403` | The API key is not allowed to access widget disclosure APIs |
| `404` | `userId` does not belong to the project associated with the API key |

...
<Note>
  A non-empty `outstanding_disclosures` array does not by itself block widget session creation. For widget ACH cashout, filter this array for Electronic Funds Transfer disclosure type `6`, present that document, and record its acceptance. Types `1` through `5` can be ignored for widget eligibility. See [Disclosure Agreements](/widget/disclosures).
</Note>

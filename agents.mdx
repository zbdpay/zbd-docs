...
description: "Build agent-native Bitcoin flows with axo, agent-pay, and agent-fetch."
---

The ZBD Agents toolkit gives you an end-to-end stack for agentic payments: a wallet CLI (`axo`), an L402 client (`agent-fetch`), an L402 server middleware (`agent-pay`), and supporting apps for identity and operations.

<Info>
  Want the fastest hands-on path? Start with the runnable scripts in `axobot-pay/examples/http-server.mjs` and `axobot-fetch/examples/zbd-agent-fetch.mjs`.
</Info>

<CardGroup cols={2}>
  <Card title="Start in Minutes" icon="rocket" href="/agents/quickstart">
    Install one package, run `axo init`, and execute your first paid request.
  </Card>
  ...
  <Card title="Ship Agent Skills" icon="wand-magic-sparkles" href="/agents/skills">
    Package `axo` workflows into reusable skills for IDE agents and OpenClaw-compatible runtimes.
  </Card>
</CardGroup>
...
<Tabs>
  ...
  2. Add payment-gated routes with [agent-pay](/agents/axobot-pay)
  3. Pay those routes with [agent-fetch](/agents/axobot-fetch) or `axo fetch`
  </Tab>
...
<CardGroup cols={2}>
  <Card title="agent-wallet" icon="github" href="https://github.com/zbdpay/agent-wallet">
    CLI wallet and command router for ZBD agent payments (`axo`).
  </Card>
...
<Info>
  `axo` is the binary shipped by `@axobot/cli`. It is designed for JSON-first CLI interoperability with agent runners.
</Info>
...
<AccordionGroup>
  ...
  <Accordion title="Autonomous data acquisition" icon="database">
    Use `axo fetch` to allow agents to pay per request and retrieve premium content in one command.
  </Accordion>

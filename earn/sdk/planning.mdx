...
Both modes use the same SDK methods (`SendReward`, `GetBalance`, `ShowModal`). The difference is the payout options available to players and — if you deliver rewards **server-side** — the `currency` field in your API calls. Client-side integrations never set a currency; the mode configured for your app determines it.
...
| Player payout options | Bank transfer (ACH), gift cards, Cash App, Speed Wallet, ZBD | Gift cards, Cash App, Speed Wallet, Bitcoin |
| Audience fit | Mainstream mobile, casual | Bitcoin-native, crypto-native |
| `currency` field (server-side API calls) | `POINT` | `MSATS` |

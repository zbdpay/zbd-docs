...
In this example, NextAuth expects a callback in the path. For example, your production environment would be `https://example.com/api/auth/callback/zbd`. Therefore we must add this to the ZBD Developer Dashboard like so:

...
| [Under the hood: ZBD API in your game](#under-the-hood) |The backend part of using ZBD.|- Store API keys on a secure server! <br/><br/> - Recommended ZBD integration: Log in with ZBD (Send to Email is deprecated)|
...
There are a few different ways to integrate the ZBD API. We recommend using Log in with ZBD for new integrations:

1. **Log in with ZBD** - Recommended approach for new integrations
...
2. ~~Send to Email~~ - **DEPRECATED** (scheduled for removal by end of February 2026)
    + Not recommended for new integrations
    + See Log in with ZBD instead

### ~~The simple way: Send to Email~~ (DEPRECATED)

<Warning>
**DEPRECATED:** The Send to Email endpoint is deprecated and scheduled for removal by the end of February 2026. Please use **Log in with ZBD** for new integrations instead.
</Warning>

This section is maintained for existing integrations only.

<Info>**The user's perspective**: As simple as writing their email!</Info>

#### Using the Send to Email API (Deprecated)
